...
- Runtime owns topology caching for rooms, devices, groups, scenes, and automations. Direct control and state/scene execution use the cache automatically when available.
...
- For fast user experience, ordinary control should go directly to the final semantic intent with target words and room qualifiers. Runtime will use cached topology and return `clarification_required` only when the target is genuinely missing or ambiguous.
...
- If the shape is unclear before a write, call local-only `intent.explain`; in its parameters object, set the `intent` field to the target semantic intent. Use `result.intentExplanation.payloadGuide` as the objective contract. This does not require cloud access and should be faster than repeated failed writes.
