...
- If the user asks to create a new home and immediately design it, first create/select the home through `home.create`, then route the selected products, rooms, slots, groups, scenes, and automations as HouseMeta to `lighting.design.import`. Do not require real devices to be installed before creating design slots.
