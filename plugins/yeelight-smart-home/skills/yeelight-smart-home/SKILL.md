...
13. Operation lessons are not user preferences. After any failed, blocked, unsupported, confusing, slow, or workaround-based Runtime/Skill attempt, first decide whether the cause is a fixable CLI bug, stale Skill rule, unclear public contract, or capability-description problem. Do not save those as user operation lessons. Record `operation.lesson.record` only for confirmed reusable Runtime behavior, stable cloud boundary, payload shape, fallback, or fast path that cannot be fixed in the current Skill/CLI flow.
