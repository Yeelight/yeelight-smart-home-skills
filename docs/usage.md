...
For hardware, `live-auto` revalidates the protected EU binding and resolves to the explicitly bound four-light quadrant proxy or eighteen-light topology. Replacing the four-light binding with the selected eighteen-bulb binding is a one-time operator action; the Host launch command and visitor routes remain unchanged. A missing or invalid live binding fails closed. Mock results are labelled as deterministic eighteen-light parity and must not be presented as live IFA hardware validation.
