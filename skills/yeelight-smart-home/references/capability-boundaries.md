...
- Non-enabled actions are not missing by default; they may be manual guidance, owner-review blocked, app-only, credential-blocked, LAN-only, or unsupported by the semantic Runtime adapter.
...
| Caller confirmation required | For destructive or permission-sensitive operations, get explicit agreement in conversation before the semantic Runtime call. |
...
- R2 persistent configuration: use direct semantic Runtime execution after normal target validation. Use dry-run first only when a preview is useful.
- R2 reviewed delete: `favorite.delete`, `favorite.batch_delete`, and single-target or capped batch delete for room, area, group, scene, and automation are direct semantic Runtime executions after explicit user request and target resolution.
- R3 high-impact operation: ask for explicit chat confirmation first, then call the semantic Runtime intent directly. Current reviewed examples include home delete, gateway delete, device delete/remove, device unbind, member removal, home ownership transfer, and leaving a shared home.
...
- Do not enable unreviewed batch delete, factory reset, broad member changes, or large bulk operations without a semantic Runtime adapter.
- `device.remove`, `device.unbind`, `gateway.delete`, `home.delete`, `home.member.remove`, `home.member.transfer`, and `home.member.quit` are reviewed R3 semantic adapters. Confirm intent in chat first, then call Runtime directly.
- `home.member.invite`, `home.member.accept_share`, `home.member.configure`, and `gateway.configure` are reviewed semantic adapters and must not be replaced with raw share, role, or gateway API calls. Accept-share uses the current local account as recipient; never pass or infer another user's uid.
