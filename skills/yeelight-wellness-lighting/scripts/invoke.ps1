...
function Read-BoundedRequestPayload {
  $Buffer = [char[]]::new($MaxRequestBytes + 1)
  $Count = 0
  ...
  if ($Count -gt $MaxRequestBytes) {
    Write-InvalidRequest
  }
  $Payload = [string]::new($Buffer, 0, $Count)
  if ([System.Text.Encoding]::UTF8.GetByteCount($Payload) -gt $MaxRequestBytes) {
    Write-InvalidRequest
  }
  return $Payload
}
...
function Invoke-Runtime($RuntimeCommand, $Gate, [string] $Payload) {
  Assert-RuntimeCompatible $RuntimeCommand
  $RuntimePayload = if ($Gate.IsWrite) { Remove-ExecutionGate $Gate.Request } else { $Payload }
...
$RequestPayload = Read-BoundedRequestPayload
$Gate = Test-RequestGate $RequestPayload

if ($env:YEELIGHT_HOME_BIN) {
  if (Test-Path $env:YEELIGHT_HOME_BIN) {
    Invoke-Runtime $env:YEELIGHT_HOME_BIN $Gate $RequestPayload
  }
...
if ($Command) {
  Invoke-Runtime "yeelight-home" $Gate $RequestPayload
}
...
foreach ($Candidate in ($CandidatePaths | Select-Object -Unique)) {
  if ($Candidate -and (Test-Path $Candidate)) {
    Invoke-Runtime $Candidate $Gate $RequestPayload
  }
}
