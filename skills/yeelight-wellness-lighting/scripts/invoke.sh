...
read_request_payload() {
  request_with_sentinel=$(
    dd iflag=fullblock bs=$((MAX_REQUEST_BYTES + 1)) count=1 2>/dev/null || exit 1
    printf '%s' '__YEELIGHT_WELLNESS_EOF__'
  ) || request_invalid
  request_payload=${request_with_sentinel%__YEELIGHT_WELLNESS_EOF__}
  request_size=$(printf '%s' "$request_payload" | wc -c | tr -d '[:space:]')
  case "$request_size" in
    ''|*[!0-9]*) request_invalid ;;
  esac
  [ "$request_size" -le "$MAX_REQUEST_BYTES" ] || request_invalid
}
...
invoke_runtime() {
  ...
  if [ "$kind" = "write" ]; then
    runtime_payload=$(strip_execution_gate "$original_payload") || request_invalid
    printf '%s' "$runtime_payload" | "$runtime_bin" invoke --stdin "$@"
  else
    printf '%s' "$original_payload" | "$runtime_bin" invoke --stdin "$@"
  fi
  exit $?
}
...
if [ -n "${YEELIGHT_HOME_BIN:-}" ]; then
  if [ -x "$YEELIGHT_HOME_BIN" ]; then
    invoke_runtime "$YEELIGHT_HOME_BIN" "$request_payload" "$request_kind" "$@"
  fi
  cat <<'JSON'
...
if command -v yeelight-home >/dev/null 2>&1; then
  invoke_runtime "yeelight-home" "$request_payload" "$request_kind" "$@"
fi
...
for candidate in \
  ...
  if [ -x "$candidate" ]; then
    invoke_runtime "$candidate" "$request_payload" "$request_kind" "$@"
  fi
done
