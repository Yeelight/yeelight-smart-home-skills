...
import { defaultPropertyValue, PropertyValueControl } from "../../components/property-value-control";
...
export function SceneEditor({ mode, scene, targets, onPreview, onSave, onCancel, onNavigate }: Props) {
  ...
  const updateDraft: DraftUpdate = (value) => { setIssues([]); setDraft(value); };
  ...
  return <section className="scene-editor" data-unsaved-editor aria-labelledby="scene-editor-title" aria-busy={Boolean(busy)}>
    ...
    <div className="scene-editor-body">{step === 0 ? <Basics draft={draft} issues={issues} update={updateDraft} /> : step === 1 ? <Actions draft={draft} targets={targets} issues={issues} update={updateDraft} /> : <Review draft={draft} />}</div>
...
function Actions({ draft, targets, issues, update }: { draft: SceneDraft; targets: SceneTargetOption[]; issues: Issue[]; update: DraftUpdate }) {
  const add = () => { const target = targets[0]; if (!target) return; const property = target.properties[0]; update((current) => ({ ...current, actions: [...current.actions, { targetType: target.type, targetId: target.id, targetName: target.name, rank: current.actions.length, set: { [property]: defaultPropertyValue(property) } }] })); };
