...
function GatewayEditor({ gateway, rooms, saving, error, onClose, onSave }: { gateway: Gateway; rooms: Room[]; saving: boolean; error?: string; onClose: () => void; onSave: (input: { name: string; roomIds: string[] }) => Promise<void> }) {
  ...
  const dialogRef = useRef<HTMLElement>(null); const nameRef = useRef<HTMLInputElement>(null);
  const dirty = name !== gateway.name || JSON.stringify([...roomIds].sort()) !== JSON.stringify([...(gateway.roomIds || [])].sort());
  const close = () => { if (!saving && (!dirty || window.confirm("放弃尚未保存的网关修改？"))) onClose(); };
  useEffect(() => { const previous = document.body.style.overflow; document.body.style.overflow = "hidden"; nameRef.current?.focus(); const onKey = (event: KeyboardEvent) => { if (event.key === "Escape") close(); if (event.key !== "Tab" || !dialogRef.current) return; const controls = [...dialogRef.current.querySelectorAll<HTMLElement>("button:not(:disabled), input:not(:disabled)")]; const first = controls[0]; const last = controls.at(-1); if (event.shiftKey && document.activeElement === first) { event.preventDefault(); last?.focus(); } else if (!event.shiftKey && document.activeElement === last) { event.preventDefault(); first?.focus(); } }; document.addEventListener("keydown", onKey); return () => { document.body.style.overflow = previous; document.removeEventListener("keydown", onKey); }; }, [dirty, saving]);
  return <div className="gateway-dialog-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}><section ref={dialogRef} className="gateway-dialog" role="dialog" aria-modal="true" aria-labelledby="gateway-dialog-title"><header><div><small>网关设置</small><h3 id="gateway-dialog-title">编辑 {gateway.name}</h3></div><button type="button" className="icon-button" onClick={close} aria-label="关闭网关编辑"><X size={18} /></button></header><label>网关名称<input ref={nameRef} value={name} maxLength={40} onChange={(event) => setName(event.target.value)} /></label><fieldset><legend>覆盖空间</legend><div className="gateway-room-options">{rooms.map((room) => <label key={room.id}><input type="checkbox" checked={roomIds.includes(room.id)} onChange={() => setRoomIds((current) => current.includes(room.id) ? current.filter((id) => id !== room.id) : [...current, room.id])} /><span>{room.name}</span></label>)}</div></fieldset>{error && <div className="gateway-dialog-error" role="alert"><CircleAlert size={18} />{error}</div>}<footer><button type="button" className="secondary-action" onClick={close}>取消</button><button type="button" className="primary-action" disabled={saving || !dirty || !name.trim()} onClick={() => void onSave({ name: name.trim(), roomIds })}>{saving ? <RefreshCw className="spin" size={17} /> : <CheckCircle2 size={17} />}{saving ? "正在保存" : "检查并保存"}</button></footer></section></div>;
}
${deleteComponent}
