...
该 Skill 仍可从 Yeelight 官方 publisher namespace 安装，但这个渠道当前是可选且滞后的 `0.1.9`。最新版 `{{version}}` 请使用 GitHub Release。
...
- 状态：`{{version}}` 可选阻塞。ClawHub 安全包已排除无扩展名的 `scripts/invoke`，但 ClawHub 仍返回 `skillId/versionId invalid value`；GitHub Release 是当前最新版事实渠道。
...
- 可选且滞后：ClawHub 仍可安装 `0.1.9`，但不阻断 `{{version}}` 及后续版本发布。
